...
void WebScraper::saveRecipe(const std::string &url, const std::string &directory)
{
    ...
    if (fileStream.is_open())
    {
        ...
        fileStream << "url: " << recipeImageUrl << std::endl;
        fileStream.close();
    }
    else
    {
...
void WebScraper::parallelScraping(int threadsN)
{
    ...
    for (auto &entry : this->baseSections)
    {
        Task initialTask{entry.first, TaskType::FetchRecipes, ""};
    ...
    auto worker = [&]()
    {
        ...
        while (true)
        {
            ...
            if (task.type == TaskType::FetchRecipes)
            {
...

                {
                    ...
                    {
                        taskQueue.push({href, TaskType::SaveRecipeDetails, dirName});
                    }
                }
            }
            else if ((task.type == TaskType::SaveRecipeDetails) && (task.directory != ""))
            {
